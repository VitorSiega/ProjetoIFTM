...
// return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Preencha todos os
// dados");
// }
...
// public ResponseEntity<String> atualizarJogo(@PathVariable Long id,
// @RequestBody GamesDTO gamesDto) {
// try {
// if (gamesRepository.findById(id).isEmpty()) {
// return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Jogo não
// encontrado");
// }
// if (gamesDto.nome().isEmpty() || gamesDto.descricao().isEmpty() ||
// gamesDto.local().isEmpty() || gamesDto.responsaveis().isEmpty() ||
// gamesDto.situacao().isEmpty()) {
// return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Preencha todos os
// dados");
// }
